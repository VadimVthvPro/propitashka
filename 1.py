...
async def handle_photo(message:Message, state: FSMContext):
    ...
    img = await tf.keras.utils.load_img("photo.jpg", target_size=(img_height, img_width))
    img_array = await tf.keras.utils.img_to_array(img)
    img_array = await tf.expand_dims(img_array, 0)
    predictions =await model.predict(img_array)
    score = await tf.nn.softmax(predictions[0])
    lol = await str(class_names[np.argmax(score)])
    translator = await Translator(from_lang="en", to_lang="ru")
    await name_a.append(translator.translate(lol).title())
    await state.set_state(REG.grams)
    await bot.send_message(message.chat.id, text="Введи, сколько грамм было в этом блюде")
