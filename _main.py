...
async def start(message: Message, state: FSMContext):
    ...
    cursor.execute(
                 f"""
                    ...
                    DO $$
                    BEGIN
                        IF EXISTS (SELECT * FROM user_lang) THEN
                            UPDATE user_lang SET lang='{lenguages[data['leng']]}' WHERE user_id = {message.from_user.id};
                        ELSE
...
async def entrance(message: Message, state: FSMContext):
    try:
        ...
        *
        ...
        """)
        user_data_ = cursor.fetchone()



        imt, imt_str, cal, weight =  float(user_data[1]), user_data[2], float(user_data[3]), user_data[6]
        height =  float(user_data_[0])
        conn.commit()
...
async def wei(message: Message, state: FSMContext):
    try:
        ...
        cursor.execute(f"""
            INSERT INTO user_health (user_id,imt,imt_str,cal,date, weight) VALUES ({message.from_user.id},{imt},'{imt_using_words}',{cal},'{datetime.datetime.now().strftime('%Y-%m-%d')}', {weight} )
            ;
            ...
            BEGIN
                ...
                IF EXISTS (SELECT * FROM user_main WHERE user_id = {message.from_user.id}) THEN 
                    UPDATE user_main SET height={height}  WHERE user_id = {message.from_user.id};
                ELSE
                    INSERT INTO user_main 
                    (user_id, user_name,user_sex,height,date_of_birth) VALUES ({message.from_user.id},'{message.from_user.first_name}','{sex}',{height},{age})
                    ;
                END IF;
...
async def ai(message: Message, state: FSMContext):
    ...
    cursor.execute(
        f"SELECT user_sex, date_of_birth, height FROM user_main WHERE user_id = {message.from_user.id} "
    )
    sex, age, height = cursor.fetchone()
    cursor.execute(
        f"SELECT imt, weight FROM user_health WHERE user_id = {message.from_user.id}"
    )
    imt, weight = cursor.fetchone()
...
async def start2(message: Message, state: FSMContext):
    ...
    UPDATE user_lang SET lang='{lenguages[data['leng2']]}' WHERE user_id = {message.from_user.id};
